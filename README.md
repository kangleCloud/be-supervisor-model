...
数据库真实列只保留 `config_path/file_name/content_program_name`；`configName/programName` 仅作为模板命名语义，不再作为数据库列存在。
...
- `DELETE /admin/api/supervisor/services/{contentProgramName}` 是硬删除；固定先 `stop`，再删除当前 `.ini` 与数据库记录，并保留 `.bak`
...
- `DELETE /admin/api/supervisor/services/{contentProgramName}?host=10.1.0.104`（硬删除，固定先停止再删除）
