...
class TemplateService:
    ...
    def render(self, payload: ServiceUpsertRequest) -> RenderedConfig:
        """渲染 Supervisor 模板。"""
        program_name = self.build_program_name(payload.job_name, payload.module_name)
        config_name = self.build_config_name(payload.config_name, program_name)
        jar_name = ensure_safe_name(payload.jar_name or self.build_default_jar_name(payload.module_name), "jarName")
        ensure_valid_port(payload.port)
        ...
        return RenderedConfig(program_name=program_name, config_name=config_name, content=content)
    ...
    def parse(content: str) -> ParsedConfig:
        ...
        parser.optionxform = str
        parser.read_string(content)
        section_name = next(iter(parser.sections()))
